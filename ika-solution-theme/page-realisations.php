  ...
  <main>
    <section class="relative overflow-hidden bg-ikaBlueDark pt-36 text-white sm:pt-40">
      ...
      <div class="relative mx-auto max-w-7xl px-4 pb-20 sm:px-6 lg:px-8">
        <div class="max-w-4xl">
          <a href="<?php echo esc_url( home_url( '/#realisations' ) ); ?>" class="inline-flex items-center gap-2 rounded-full border border-white/25 bg-white/10 px-5 py-3 text-sm font-black text-white transition hover:bg-white hover:text-ikaBlue"><svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M19 12H5"/><path d="m12 19-7-7 7-7"/></svg><?php echo esc_html( ika_opt( 'ika_realisations_hero_back' ) ); ?></a>
          <p class="mt-8 text-sm font-black uppercase tracking-[0.2em] text-red-200"><?php echo esc_html( ika_opt( 'ika_realisations_hero_eyebrow' ) ); ?></p>
          <h1 class="mt-5 text-4xl font-black leading-[1.05] tracking-normal sm:text-5xl lg:text-6xl"><?php echo esc_html( ika_opt( 'ika_realisations_hero_title' ) ); ?></h1>
          <p class="mt-6 max-w-3xl text-lg leading-8 text-white/80 sm:text-xl"><?php echo esc_html( ika_opt( 'ika_realisations_hero_text' ) ); ?></p>
        </div>
      </div>
    </section>
    ...
    <section id="realisations" class="bg-white py-20 sm:py-28">
      <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div class="max-w-3xl">
            <p class="text-sm font-black uppercase tracking-[0.2em] text-ikaRed"><?php echo esc_html( ika_opt( 'ika_realisations_section_eyebrow' ) ); ?></p>
            <h2 class="mt-4 text-4xl font-black tracking-normal text-ikaBlueDark sm:text-5xl"><?php echo esc_html( ika_opt( 'ika_realisations_section_title' ) ); ?></h2>
          </div>
          <a href="<?php echo esc_url( home_url( '/#contact' ) ); ?>" class="inline-flex w-fit shrink-0 rounded-full bg-ikaRed px-6 py-3 text-sm font-black text-white shadow-clean transition hover:bg-red-700"><?php echo esc_html( ika_opt( 'ika_realisations_section_cta' ) ); ?></a>
        </div>

        <div id="filterBar" class="mt-8 flex flex-wrap gap-2">
          <button class="filter-btn rounded-full bg-ikaRed px-5 py-3 text-sm font-black text-white transition hover:bg-red-700" data-filter="all"><?php echo esc_html( ika_opt( 'ika_realisations_filter_all' ) ); ?></button>
